  ...
  disputes:any[] = [];
                ...
                for(const i in resp){
                  ...
                  /*=============================================
						...
						if(resp[i].picture != undefined){
							...
							this.picture = `assets/img/users/default/defaultuser.png`;
						}
						/*=============================================
						Método de registro
						=============================================*/
    	...
    	if(value != ""){
	    	...
	    	.subscribe(resp1=>{
      			...
      			.subscribe(resp2=>{
					...
					Sweetalert.fnc("success", "La constraseña ha sido cambiado satisfactoriamente", "account")
				})
	    	}, err =>{
    ...
    validateImage(e){
        ...
        if(this.image["type"] !== "image/jpeg" && this.image["type"] !== "image/png"){
	    	Sweetalert.fnc("error", "La imagen debe ser in formato JPG o PNG", null)
	    	return;
        }
        /*=============================================
        Validamos el tamaño
    ...
    uploadImage(){
    	...
    	.subscribe(resp =>{
    		if(resp["status"] == 200){
    			...
    			.subscribe(resp=>{
    				if(resp["picture"] != ""){
    					Sweetalert.fnc("success", "¡Su foto de perfil ha sido actualizado!", "account")
    				}
    			})
    		}
    	})
