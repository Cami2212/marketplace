...
export class ReviewsComponent implements OnInit {
  ...
  reviews:any[] = [];
  	...
  	ngOnInit(): void {
        ...
        for(let i = 0; i < this.reviews[0].length; i++){
        ...
        JSON.parse(this.childItem["reviews"]).forEach(rev =>{
        	...
        	this.arrayReview.push(rev.review)
        })

        //Ordenamos el array de mayor a menor
	    this.arrayReview.sort();
	    ...
	    this.arrayReview.forEach((value, index, arr)=>{
	    ...
	    for(let i = 5; i > 0; i--){
	    	...
	    	let starPercentage = Math.round((objectStar[i] * 100)/ this.arrayReview.length)
